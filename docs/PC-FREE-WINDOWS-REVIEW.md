...
أظهرت دورة Windows الأخيرة أن build من المصدر نجح، وأن تثبيت SQL prerequisite نجح، لكن المثبت أعاد تشغيل SQL setup فوق instance موجودة ولم يكتمل بسبب رمز `-2068643838` (`0x84B30002`). تم تعديل `EnsureSqlExpress` بحيث يفحص وجود خدمة `MSSQL$SQLEXPRESS`، وينتظر اتصال SQL حتى 5 دقائق بدلاً من إعادة تشغيل setup فوق خدمة موجودة، ثم يرجع رسالة تشخيصية إذا لم تصبح جاهزة. الدورة التالية ستتحقق من هذا الإصلاح.
